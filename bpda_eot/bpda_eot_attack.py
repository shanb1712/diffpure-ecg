...
class BPDA_EOT_Attack():
    ...
    def predict(self, X, y, requires_grad=True, reps=1, eot_defense_ave=None, eot_attack_ave='loss'):
        if requires_grad:
            y_pred, output = self.model(X, mode='classify')
        else:
            with torch.no_grad():
                y_pred, output = self.model(X.data, mode='classify')

        # y_pred = self.eot_defense_prediction(logits.detach(), reps, eot_defense_ave)
        correct = torch.eq(y_pred, y).all(1)
        loss = self.eot_attack_loss(output, y, reps, eot_attack_ave)
    ...
    def purify_and_predict(self, X, y, purify_reps=1, requires_grad=True):
        X_repeat = X.repeat([purify_reps, 1, 1])
    ...
    def attack_batch(self, X, y):
        ...
        defended = self.eval_and_bpda_eot_grad(X, y, torch.ones_like(y.any(1)).bool(), False)[0]
